...
const philosophyCards = [
  ...
  { id: "03", text: "We act with urgency to ensure progress and deliver results ef10:00 AMficiently. Every moment matters.", hashtag: "#URGENCY", color: "bg-yellow-500", icon: <Target className="w-8 h-8 text-white" /> },
  ...
  { id: "07", text: "We value 10:00 AMfresh ideas, bold solutions, and the freedom to imagine what’s next.", hashtag: "#CREATIVITY", color: "bg-indigo-500", icon: <Lightbulb className="w-8 h-8 text-white" /> },
...
const OurValues = ({ theme }) => {
  ...
  return (
    <WarpBackground className={`${isDark ? "bg-black text-white" : "bg-gray-100 text-gray-900"} border-0 rounded-none !p-10`}>
          ...
          {philosophyCards.map((card, index) => (
            <motion.div
              ...
              className={cn(
                ...
                card.color,
                "w-full md:w-[60%] lg:w-[100%]" // Increased card width
              )}
            ...
            >
              <Card className="border-none bg-transparent">
                <CardContent className="flex flex-col gap-4">
                  ...
                  <p className="text-base md:text-lg">{card.text}</p> {/* Adjusted font size */}
